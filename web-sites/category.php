...
if (isset($_REQUEST['id']))
{
	...
	$smarty->assign('c_tree',       get_child_tree($cat_id));//获取当前栏目的下级栏目
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		//小七修改
		 $arr[$row['goods_id']]['goumairshu']             = get_goods_ordernum($row['goods_id']);
		if($row['market_price'] !='0' and $row['shop_price'] !='0'){
		$arr[$row['goods_id']]['zhekou_price'] = round(($row['shop_price']/$row['market_price'])*100)/10+"";
		}	 
		 //小七修改结束
