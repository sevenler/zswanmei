...
(function($){
    imageSlidePlayer = function(object,config){
        ...
        this.resetclass =function(obj,i){
            ...
            if($.browser.msie && $.browser.version == 6.0){
                $(obj).css("filter","progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+RM_IMGDIR+"/mall_dot.png');");
                $(obj).eq(i).css("filter","progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+RM_IMGDIR+"/mall_dot_hover.png');");
            }else{
                $(obj).css({background:"url("+RM_IMGDIR+"/mall_dot.png) no-repeat"});
                $(obj).eq(i).css({background:"url("+RM_IMGDIR+"/mall_dot_hover.png) no-repeat"});
            }
        }
        this.factory();
    }
...
(function($){
    HtmlSlidePlayer = function(object,config){
        ...
        this.factory = function(){
            //元素定位
            $(this.obj).css({position:"relative",zIndex:"0",overflow:"hidden", height: $(this.obj + " ul").height() + "px" });
            $(this.obj).prepend("<div class='control' style='position:absolute;z-index:20;left:"+this.config.right+"px;bottom:"+this.config.bottom+"px'></div>");
        ...
        //图片渐影
        this.slide = function(){
            $(this.obj + " .control a").lazyhover(function(){
                _this.j = $(this).text() - 1;
                _this.n = _this.j;
                if (_this.j >= _this.count){return;}
        ...
        this.autoplay = function(){
            _this.n = _this.n >= (_this.count - 1) ? 0 : ++_this.n;
            $(_this.obj + " .control a").eq(_this.n).triggerHandler('mouseover');
        }
        //翻页函数
        this.resetclass =function(obj,i){
            $(obj).css({'float':"left",marginRight:"15px",width:"11px",overflow:"hidden",height:"11px",lineHeight:"100px",textAlign:"center",fontWeight:"800",fontSize:"12px",color:"#fff",cursor:"pointer",fontFamily:"Tahoma"});
            ...
            if($.browser.msie && $.browser.version == 6.0){
                $(obj).css("filter","progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+RM_IMGDIR+"/mall/mall_dot.png');");
                $(obj).eq(i).css("filter","progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+RM_IMGDIR+"/mall/mall_dot_hover.png');");
            }else{
                $(obj).css({background:"url("+RM_IMGDIR+"/mall/mall_dot.png) no-repeat"});
                $(obj).eq(i).css({background:"url("+RM_IMGDIR+"/mall/mall_dot_hover.png) no-repeat"});
            }
        }
        this.factory();
    }
