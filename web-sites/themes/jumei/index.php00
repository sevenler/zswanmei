...
//小七修改
function tjiansp($fenlei,$type,$kaishi,$jieshu)
{ 
 switch ($type)
...
$timestamp=time();
$sql = "SELECT * FROM  ecs_goods WHERE is_on_sale = 1 AND  is_alone_sale = 1 $sql AND is_delete = 0 and cat_id = $fenlei ORDER BY `add_time` DESC LIMIT $kaishi , $jieshu";
     $res = $GLOBALS['db']->query($sql);
    $list = array();
  while ($row = $GLOBALS['db']->fetchRow($res))
    {
